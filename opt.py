    ...
    for i in range(len(layers)):
        ...
        if args.nearest:
            ...
            for name in subset:
                ...
                # print(f"layer {i} torch sum of {name}",torch.sum(W))
                ...
                if args.not_quantize_outliers:
                    import pickle
                    with open("/groups/huanruiyang/dongweiw/controllableQ/LORA_FT/finetuned/OPT-2.7B/epoch1outlier_dict_2.0", 'rb') as f:
        ...
        #         with open("/groups/huanruiyang/dongweiw/controllableQ/visualizations/quantization_errors_finetuned.txt", "a") as file:
        #             file.write(
        #                 f"quantization error of layer {i} {name}: {torch.norm(W - subset[name].weight.data, p=2)}\n")
        # with open("/groups/huanruiyang/dongweiw/controllableQ/visualizations/quantization_errors_finetuned.txt",
        #           "a") as file:
        #     file.write(
        #         f"Sum of quantization error of layer {i} : {layer_error}\n")
...
if __name__ == "__main__":
    ...
    if args.load:
        ...
        model = get_opt(model=args.model)
        # model = load_model(model = args.model,model_type="opt")
        model.eval()
        ...
        for dataset in datasets:
            ...
            if args.multilingual:
                dataloader, testloader = get_mlg_loaders(
                    dataset, seed=args.seed, model=args.model, seqlen=512
                )
            else:
                dataloader, testloader = get_loaders(
